            ...
            if image[i, j] == 0 and i - 1 >= 0 and j - 1 >= 0 and i + 1 < image.shape[0] and j + 1 < image.shape[1]:
                for y in range(-1, 2):
                    for x in range(-1, 2):
                        ...
                        if [offset_i, offset_j] != [i, j] and struct[y + 1][x + 1] == 1 and image[offset_i, offset_j] == 255:
            ...
            if image[i, j] == 255 and i - 1 >= 0 and j - 1 >= 0 and i + 1 < image.shape[0] and j + 1 < image.shape[1]:
                for y in range(-1, len(struct) - 1): 
                    for x in range(-1, len(struct) - 1): 
                        ...
                        if [offset_i, offset_j] != [i, j] and struct[y + 1][x + 1] == 1 and image[offset_i, offset_j] == 0:
            ...
            if image[i, j] == 0 and label_list[i][j] == 0: 
                ...
                while len(queue) > 0: 
                    ...
                    if 0<item[0]<image.shape[0]-1 and 0<item[1]<image.shape[1]-1:
                        if image[item[0] - 1, item[1]] == 0 and label_list[item[0] - 1][item[1]] == 0: # Checks if the neighbour above is a foreground pixel and is currently unlabelled
                        ...
                        if image[item[0] + 1, item[1]] == 0 and label_list[item[0] + 1][item[1]] == 0: # Checks if the neighbour below is a foreground pixel and is currently unlabelled
                            queue.append([item[0] + 1, item[1]])
                            label_list[item[0] + 1][item[1]] = curlab
                        if image[item[0], item[1] - 1] == 0 and label_list[item[0]][item[1] - 1] == 0: # Checks if the neighbour to the left is a foreground pixel and is currently unlabelled
                            queue.append([item[0], item[1] - 1])
                            label_list[item[0]][item[1] - 1] = curlab
                        if image[item[0], item[1] + 1] == 0 and label_list[item[0]][item[1] + 1] == 0:  # Checks if the neighbour to the right is a foreground pixel and is currently unlabelled
                            queue.append([item[0], item[1] + 1])
                            label_list[item[0]][item[1] + 1] = curlab
                curlab += 1 
    return label_list
...
def calculate_area(label_list, curlab): 
    ...
    for x in range(0, len(label_list)): 
        for y in range(0, len(label_list[0])): 
            if label_list[x][y] == curlab: # Checks if the current label at this index position matches the one we are trying to count
                area += 1
    return area
...
def paint_labels(image, label_list): 
    ...
    for x in range(0, image.shape[0]): 
        for y in range(0, image.shape[1]): 
                pImage[x, y] = label_list[x][y] * 255 # Updates the labelled pixel by multiplying it by 255 which sets the foreground colours to white and background to black
    return pImage
...
def remove_smallest_areas(label_list): 
    ...
    if len(unique_labels) > 2: 
        ...
        for i in range(len(unique_labels)): 
            areas.append(calculate_area(label_list, unique_labels[i])) # Calls a method to calculate the area of the currently selected label/component
    ...
    for i in range(0, len(image)):
        for j in range(0, len(image[0])):
            if image[i][j] == 1:
                ...
                total_j += j
                total_i += i
    return [int(total_i / pixel), int(total_j / pixel)] # The sum of foreground i position divided by the count of foreground pixels gives the average i, likewise for j
...
def calculate_radius(image, centroid): 
    ...
    for i in range(centroid[0] - 1, 0, -1): # Loops from the centroid y position to the top of the image
        ...
        if not found[1]:
            radius[1] += 1
        if image[i, centroid[1]] == 255 and image[i + 1, centroid[1]] == 0: # Checks if the current pixel is of a foregorund colour and the previous pixel was background, then the inner edge has been found
            found[0] = True 
        if image[i, centroid[1]] == 0 and image[i + 1, centroid[1]] == 255: # Checks if the current pixel is of a background colour and the previous pixel was foreground, then the outer edge has been reached
            found[1] = True 
...
def oring_result(image, centroid, radius): 
    ...
    for i in range(0, image.shape[0]):
        for j in range(0, image.shape[1]):
            ...
            if circle_center(j, i, centroid[1], centroid[0], radius[1]) and not circle_center(j, i, centroid[1], centroid[0], radius[0]):
    ...
    for i in range(0, image.shape[0]):
        for j in range(0, image.shape[1]):
            ...
            if image[i, j] == 0 and shape[i, j] == shape_pixel or image[i, j] == 255 and shape[i, j] == 0:
                ...
                if pixel_faulty:
                    faulty_coords.append([i, j]) # Adds its coordinates to the faulty pixel coordinates list for recolouring the faulty area
...
def draw_faulty_locations(image, expected_ring): 
    ...
    for i in range(0, image.shape[0]):
        for j in range(0, image.shape[1]):
            ...
            if np.array_equal(expected_ring[i,j], np.array([255, 255, 255])) and np.array_equal(image[i, j], np.array([0, 0, 0])) or np.array_equal(expected_ring[i,j], np.array([0, 0, 0])) and np.array_equal(image[i, j], np.array([255, 255, 255])):
...
def start_process(title, image, labels, centroid, bounding_box, start, totals):
    ...
    drawn_image = paint_labels(drawn_image, labels) # Paints the labels of the components on the image which inverts the foreground and background colours
    radius = calculate_radius(drawn_image, centroid) # Calculates the 2 radius values which are the inner and outer edge of the ORing  
    result = oring_result(drawn_image, centroid, radius) # Processes the image and returns true or false for whether it passed
    finish = time.time() # Retrieves the time for when all the processing is complete
    results[2] = round((finish - start), 3) # Adds the processing time for the current ring to the total processing time count rounded to 3 decimals
    drawn_image = cv.cvtColor(drawn_image, cv.COLOR_GRAY2RGB) # Converts the image into BGR scale which allows for the use of colour rather than greyscale
    result[2] = cv.cvtColor(result[2], cv.COLOR_GRAY2RGB) # Converts the drawn pixels into BGR scale which allows to colour the fixed pixels
    ...
    if result[0] == False: 
        ...
        colour = (0, 0, 255)
        results[1] += 1
        drawn_image = draw_faulty_locations(drawn_image, result[2]) # Draws in the areas that are expected but missing in green within the circle
    else:
    ...
    drawn_image = cv.putText(drawn_image, "Time: " + str(round(finish - start, 3)), (5, 10), font, 0.4, (255, 255, 255), 1, line) # Paints the processing time text onto the image
    drawn_image = cv.putText(drawn_image, result_output, (170, 10), font, 0.4, colour, 1, line) # Prints the process result top right of the interface
    drawn_image = cv.putText(drawn_image, "Results: ", (5, 210), font, 0.4, (255, 255, 255), 1, line) # Prints results beside passed or failed for the results of tests
    drawn_image = cv.putText(drawn_image, "Passed: {}".format(results[0]), (80, 200), font, 0.3, (0, 255, 0), 1, line) # Prints the total number of passes for ORings
    drawn_image = cv.putText(drawn_image, "Failed: {}".format(results[1]), (80, 215), font, 0.3, (0, 0, 255), 1, line) # Prints the total number of fails for ORings
...
images = glob.glob(r'G:\\Microsee\\camera_calibration\\CameraCalibration-master\\src\\images\\calibrated/2/*.png')   #已校正图像
# Image counter
imageCount = 1
